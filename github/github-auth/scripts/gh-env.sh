...
elif [ -f "$HOME/.netrc" ] && grep -q "github.com" "$HOME/.netrc" 2>/dev/null; then
    # Read PAT from netrc (used when curl with -n authenticates against GitHub).
    # Guard against a truncated token (e.g. 13 chars from a mangled heredoc write
    # or security-redaction). Classic PATs are 40 chars; fine-grained are ~64.
    GITHUB_TOKEN=$(awk '/machine github.com/{flag=1; next} flag && /login/{print $2; exit}' "$HOME/.netrc")
    if [ -n "$GITHUB_TOKEN" ] && [ "${#GITHUB_TOKEN}" -ge 40 ]; then
        GH_AUTH_METHOD="curl"
    elif [ -n "$GITHUB_TOKEN" ]; then
        # Token looks truncated — discard and report as unauthenticated.
        GITHUB_TOKEN=""
    fi
fi
